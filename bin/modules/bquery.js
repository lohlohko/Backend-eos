...
//================= get table rows using query ==========================
const GetRowsByQuery = async (req, res) => {
  const result = { err: null, data: null };
  const { projectId, datasetId, tableId } = req.params;
  ...
  try {
    ...
    const query = `
      SELECT Product_Category, Sub_Category, SUM(Quantity) AS Total_Quantity 
    ...
    const [job] = await google
      .bigquery({ version: 'v2', auth: oAuth2Client })
      .jobs.query({
        projectId,
        requestBody: options,
      });
    ...
    console.error('Error fetching token:', error);
    result.err = new ForbiddenError('Invalid get access token!');
    return wrapper.response(res,'fail', result,'Invalid get access token!',ERROR.FORBIDDEN);
  }
};
module.exports = {
  token,
